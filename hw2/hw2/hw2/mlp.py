...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        non_linearity = MLP.nonlins_covertor(nonlins)
        for idx, (in_dim, out_dim) in enumerate(zip(all_dims[:-1], all_dims[1:])): 
            layers += [
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # ====== YOUR CODE: ======
        # like in tutorial ,bu without log softmax
        x = torch.reshape(x, (x.shape[0], -1))
        y_pred = self.fc_layers(x)
        return y_pred

    def nonlins_covertor(nonlins):
        non_linearity = []
        for nonlin in nonlins:
            if isinstance(nonlin,str):
                non_linearity.append(ACTIVATIONS[nonlin](**ACTIVATION_DEFAULT_KWARGS[nonlin]))
            else:
                non_linearity.append(nonlin)             
