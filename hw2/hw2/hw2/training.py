    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            temp = [ x.item() for x in torch.FloatTensor(train_result.losses).numpy() ]
            train_loss.extend(temp)
            train_acc.append(train_result.accuracy)
            temp = [ x.item() for x in torch.FloatTensor(test_result.losses).numpy() ]
            test_loss.extend(temp)
            
            # CPU training
            # test_acc.append(torch.FloatTensor(test_result.accuracy).numpy().item())
...
class ClassifierTrainer(Trainer):
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            # print(f'{num_correct=},out {len(y_pred)=}')
            # Compute loss
            loss_fn = self.loss_fn()
            batch_loss = loss_fn(y_pred,y)
            # ========================
